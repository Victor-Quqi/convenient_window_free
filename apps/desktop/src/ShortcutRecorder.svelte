...
<div class:recording class="shortcut-recorder"><button bind:this={root} aria-label={label} on:click={start} type="button">{#if recording && captured}<span class="key-row">{#each captured.split("+") as key}<kbd>{key}</kbd>{/each}</span>{:else if value}<span class="key-row">{#each value.split("+") as key}<kbd>{key}</kbd>{/each}</span>{:else}<span class="empty">{recording ? "请按任意键" : "点击录制快捷键（可单键）"}</span>{/if}</button>{#if value}<button class="clear" aria-label="清除快捷键" on:click={clear} type="button">×</button>{/if}</div>
<style>
