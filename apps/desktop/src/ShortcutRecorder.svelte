<script lang="ts">
  ...
  function handleKeydown(e: KeyboardEvent) {
    if (!recording) return;
    lastEvent = `[v3] key=${e.key ?? "?"} code=${e.code ?? "(空)"} ctrl=${e.ctrlKey ? 1 : 0} alt=${e.altKey ? 1 : 0}`;
...
<div class="shortcut-recorder-wrap"><div class:recording class="shortcut-recorder"><button bind:this={root} aria-label={label} on:click={start} on:keydown={handleKeydown} type="button">{#if recording && captured}<span class="key-row">{#each captured.split("+") as key}<kbd>{key}</kbd>{/each}</span>{:else if value}<span class="key-row">{#each value.split("+") as key}<kbd>{key}</kbd>{/each}</span>{:else}<span class="empty">{recording ? "请按任意键" : "点击录制快捷键（可单键）"} <em class="build-tag">v3</em></span>{/if}</button>{#if value}<button class="clear" aria-label="清除快捷键" on:click={clear} type="button">×</button>{/if}</div>{#if recording && lastEvent}<p class="shortcut-debug">收到：{lastEvent}</p>{/if}</div>
<style>
.shortcut-recorder-wrap{min-width:0}.build-tag{color:var(--faint);font-size:9px;font-style:normal;font-weight:600;vertical-align:super}.shortcut-debug{margin:4px 0 0;color:var(--faint);font:10.5px/1.5 ui-monospace,"Cascadia Mono",Consolas,monospace;word-break:break-all}
