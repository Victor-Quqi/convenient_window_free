<script lang="ts">
  ...
  function handleKeydown(e: KeyboardEvent) {
    if (!recording) return;
    lastEvent = `key=${e.key ?? "?"} code=${e.code ?? "(空)"} ctrl=${e.ctrlKey ? 1 : 0} alt=${e.altKey ? 1 : 0}`;
...
<style>
.shortcut-recorder-wrap{min-width:0}.shortcut-debug{margin:4px 0 0;color:var(--faint);font:10.5px/1.5 ui-monospace,"Cascadia Mono",Consolas,monospace;word-break:break-all}
